...
userFuncExec2 : Context -> List Expr -> List Statement -> UserEnv -> Array.Array ArgValue -> Result String OutVal
...
--        _ ->
--            OString "NG"
        result = eval userenv context2 stmts
    in
    case result of
        Ok (userenv_2,context_2) ->
              let
                ret = getConstant "_return_" context_2
              in
                case ret of
                   Just r ->
                       Ok r
...
userFuncExec : UserEnv -> Context -> String -> Array.Array ArgValue -> Result String OutVal
userFuncExec userenv context funcname input_args =
    ...
    case result of
        ...
        Err a ->
            ...
            Err ("NOT FOUND:::" ++ funcname)


--exec_evaluate : UserEnv -> Context -> Expr -> OutVal
exec_evaluate : UserEnv -> Context -> Expr -> Result String OutVal
...
evalWhile : Expr -> Array.Array Statement -> UserEnv -> Context -> Result String ( UserEnv, Context )
...
evalForHelp : String -> Array.Array OutVal -> Array.Array Statement -> UserEnv -> Context -> Result String ( UserEnv, Context )
           ...
           case userenv_context_3 of
               Ok (userenv_3, context_3) ->
                      if isBreak context_3 then
                          ...
                          evalForHelp name array2 stmt  userenv_3  context_3
               Err _ ->
                    userenv_context_3
...
evalFor : Expr -> Expr -> Array.Array Statement -> UserEnv -> Context -> Result String ( UserEnv, Context )
...
evalElsIfs : List ( Expr, List Statement ) -> List Statement -> UserEnv -> Context -> Result String ( UserEnv, Context )
...
evalElsIfs2 : List ( Expr, List Statement ) -> UserEnv -> Context -> Result String ( UserEnv, Context )
...
evalCaseSwitch : OutVal -> List ( Expr, List Statement ) -> UserEnv -> Context -> Result String ( UserEnv, Context )
...
evalStep : Array.Array Statement -> Int -> UserEnv -> Context -> Result String ( UserEnv, Context )
        ...
        result_main =
            ...
            case stmt of
                Just (DefVar a b) ->
                    ...
                    case b_ of
                      Ok b__ ->
                        case a of
                                Variable n ->
                                    Ok ( userenv, newConstant n b__ context )
                ...
                Just (DefUse a ) ->
                    ...
                    case func of
                         Just f_ ->
                               Ok ( userenv, addFunction a f_ context )
                         _ ->
                               Err ("DefUse undef:" ++ a) 
                ...
                Just (DefEnum a b ) ->
                    ...
                    Ok (userDefEnumAdd a b userenv context)
                ...
                Just (DefFunc a b c) ->
                    ...
                    case a of
                        Variable n ->
                            Ok (userDefFuncAdd n b c userenv context)
                ...
                Just (Assign a b) ->
                    ...
                    case b_ of
                      Ok b__ ->
                         case a of
                            Variable n ->
                               Ok ( userenv, setConstant n b__ context )
                ...
                Just (Return a) ->
                    ...
                    case a_ of
                       Ok a__ ->
                           Ok ( userenv, addConstant "_return_" a__ context_ )
                       Err _ ->
                           Err "Err Return"
                ...
                Just (Case a b) ->
                    let
                        ...
                        result =
                            case a_ of
                               Ok a__ ->
                                  evalCaseSwitch a__ b userenv context_1
                               Err str ->
                                  Err str
                ...
                --_  ->
                --    Err ("Unknow Statemwnt::" ++ (Debug.toString stmt))
                _  ->
                      Ok (userenv, context)
    in
    case result_main of
       Ok (userenv__,context__) ->
          if Array.length arr <= pos then
              ...
              if isBreak context__ || isReturn context__ then
                  ...
                  evalStep arr (pos + 1) userenv__ context__
...
eval : UserEnv -> Context -> List Statement -> Result String ( UserEnv, Context )
